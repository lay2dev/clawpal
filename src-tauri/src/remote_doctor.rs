...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct ClawpalServerPlanResponse {
    request_id: String,
    plan_id: String,
    summary: String,
    #[serde(default)]
    steps: Vec<ClawpalServerPlanStep>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct ClawpalServerPlanStep {
    #[serde(rename = "type")]
    step_type: String,
    path: Option<String>,
    value: Option<Value>,
...
pub struct RemoteDoctorRepairResult {
    ...
    round: usize,
    phase: String,
    last_plan_kind: String,
    latest_diagnosis_healthy: bool,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct RemoteDoctorProgressEvent {
    session_id: String,
    mode: String,
    round: usize,
    phase: String,
...
#[derive(Debug, Clone)]
struct ConfigExcerptContext {
    config_excerpt: Value,
    config_excerpt_raw: Option<String>,
    config_parse_error: Option<String>,
}

#[derive(Debug, Clone)]
struct RepairRoundObservation {
    round: usize,
    step_types: Vec<String>,
    diagnosis_signature: String,
    issue_summaries: Vec<Value>,
}

impl RepairRoundObservation {
    fn new(round: usize, step_types: &[String], diagnosis: &RescuePrimaryDiagnosisResult) -> Self {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct StoredRemoteDoctorIdentity {
    version: u8,
    created_at_ms: u64,
    device_id: String,
...
fn parse_target_location(raw: &str) -> Result<TargetLocation, String> {
    match raw {
...
fn remote_doctor_log_dir() -> PathBuf {
    resolve_paths().clawpal_dir.join("doctor").join("remote")
}

fn append_remote_doctor_log(session_id: &str, payload: Value) {
    let dir = remote_doctor_log_dir();
...
fn emit_progress<R: Runtime>(
    ...
    let payload = RemoteDoctorProgressEvent {
        session_id: session_id.to_string(),
        mode: "remoteDoctor".into(),
        round,
        phase: phase.to_string(),
        line: line.into(),
        plan_kind: plan_kind.map(|kind| match kind {
...
fn result_for_completion_with_warnings(
    session_id: &str,
    round: usize,
    last_plan_kind: PlanKind,
    last_command: Option<Vec<String>>,
...
fn diagnosis_issue_summaries(diagnosis: &RescuePrimaryDiagnosisResult) -> Vec<Value> {
    diagnosis
        .issues
        .iter()
        .map(|issue| {
